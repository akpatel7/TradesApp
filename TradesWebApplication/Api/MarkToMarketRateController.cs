    ...
    public class MarkToMarketRateController : ApiController
    {
        ...
        public HttpResponseMessage Post([FromUri]string tradeId, [FromUri]string newMarktoMarketRate)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    resultingTrackRecordId = PersistToDb(tradeId, newMarktoMarketRate);
                }
        ...
        private string PersistToDb(string tradeId, string mark_to_mark_rate)
        {
            
            var trade = unitOfWork.TradeRepository.Get(int.Parse(tradeId));


            if (!String.IsNullOrEmpty(mark_to_mark_rate))
            {
                var markTR = new Track_Record
                {
                    trade_id = int.Parse(tradeId),
                    track_record_type_id = 1,
                    track_record_value = decimal.Parse(mark_to_mark_rate)
                };
